<template>
  <div class="cesiumContainer-threshold-water1" id="cesiumContainer-threshold-water1">
...
onMounted(async () => {
  /*--------initializing map------------*/
  let {viewer, scene, globe, clock} = CesiumTool.initMap("cesiumContainer-threshold-water1")
  ...
  // layer1
  let vWaterArr_l1t1 = reactive<number[]>([])
  let vWaterArr_l1t2 = reactive<number[]>([])
  let vWaterArr_l1t3 = reactive<number[]>([])
  let vWaterArr_l1t4 = reactive<number[]>([])
  let vWaterArr_l1t5 = reactive<number[]>([])
  let vWaterArr_l1t6 = reactive<number[]>([])
  let vWaterArr_l1t7 = reactive<number[]>([])
  let vWaterArr_l1t8 = reactive<number[]>([])

  vWaterArr_l1t1.push(...await getVelocityData(1, 1, 'water'))
  vWaterArr_l1t2.push(...await getVelocityData(2, 1, 'water'))
  vWaterArr_l1t3.push(...await getVelocityData(3, 1, 'water'))
  vWaterArr_l1t4.push(...await getVelocityData(4, 1, 'water'))
  vWaterArr_l1t5.push(...await getVelocityData(5, 1, 'water'))
  vWaterArr_l1t6.push(...await getVelocityData(6, 1, 'water'))
  vWaterArr_l1t7.push(...await getVelocityData(7, 1, 'water'))
  vWaterArr_l1t8.push(...await getVelocityData(8, 1, 'water'))

  featureEach(pressure_l1t1_jsonData, (currentFeature, featureIndex) => {
    currentFeature.properties.pressure = vWaterArr_l1t1[featureIndex]
  })

  //sample velocity array
  const vWaterArrList_l1: number[][] = [
    vWaterArr_l1t1,
    vWaterArr_l1t2,
    vWaterArr_l1t3,
    vWaterArr_l1t4,
  ...
  const mainRendering = () => {
    ...
    let dynamicRenderController = render.renderByTimeInterpolation(cesiumFieldMap, sampleData_l1, contourOptions, renderOptions)
  }
...
.cesiumContainer-threshold-water1 {
  position: relative;
  width: 100%;
  height: 99%;
