<template>
  <div class="cesiumContainer-threshold-water4" id="cesiumContainer-threshold-water4">
...
onMounted(async () => {
  /*--------initializing map------------*/
  let {viewer, scene, globe, clock} = CesiumTool.initMap("cesiumContainer-threshold-water4")
  ...
  // layer1
  let vWaterArr_l4t1 = reactive<number[]>([])
  let vWaterArr_l4t2 = reactive<number[]>([])
  let vWaterArr_l4t3 = reactive<number[]>([])
  let vWaterArr_l4t4 = reactive<number[]>([])
  let vWaterArr_l4t5 = reactive<number[]>([])
  let vWaterArr_l4t6 = reactive<number[]>([])
  let vWaterArr_l4t7 = reactive<number[]>([])
  let vWaterArr_l4t8 = reactive<number[]>([])

  vWaterArr_l4t1.push(...await getVelocityData(1, 4, 'water'))
  vWaterArr_l4t2.push(...await getVelocityData(2, 4, 'water'))
  vWaterArr_l4t3.push(...await getVelocityData(3, 4, 'water'))
  vWaterArr_l4t4.push(...await getVelocityData(4, 4, 'water'))
  vWaterArr_l4t5.push(...await getVelocityData(5, 4, 'water'))
  vWaterArr_l4t6.push(...await getVelocityData(6, 4, 'water'))
  vWaterArr_l4t7.push(...await getVelocityData(7, 4, 'water'))
  vWaterArr_l4t8.push(...await getVelocityData(8, 4, 'water'))

  featureEach(pressure_l1t1_jsonData, (currentFeature, featureIndex) => {
    currentFeature.properties.pressure = vWaterArr_l4t1[featureIndex]
  })

  //sample velocity array
  const vWaterArrList_l4: number[][] = [
    vWaterArr_l4t1,
    vWaterArr_l4t2,
    vWaterArr_l4t3,
    vWaterArr_l4t4,
...
.cesiumContainer-threshold-water4 {
  position: relative;
  width: 100%;
  height: 99%;
