<template>
  <div class="cesiumContainer-threshold-water3" id="cesiumContainer-threshold-water3">
...
onMounted(async () => {
  /*--------initializing map------------*/
  let {viewer, scene, globe, clock} = CesiumTool.initMap("cesiumContainer-threshold-water3")
  ...
  // layer1
  let vWaterArr_l3t1 = reactive<number[]>([])
  let vWaterArr_l3t2 = reactive<number[]>([])
  let vWaterArr_l3t3 = reactive<number[]>([])
  let vWaterArr_l3t4 = reactive<number[]>([])
  let vWaterArr_l3t5 = reactive<number[]>([])
  let vWaterArr_l3t6 = reactive<number[]>([])
  let vWaterArr_l3t7 = reactive<number[]>([])
  let vWaterArr_l3t8 = reactive<number[]>([])

  vWaterArr_l3t1.push(...await getVelocityData(1, 3, 'water'))
  vWaterArr_l3t2.push(...await getVelocityData(2, 3, 'water'))
  vWaterArr_l3t3.push(...await getVelocityData(3, 3, 'water'))
  vWaterArr_l3t4.push(...await getVelocityData(4, 3, 'water'))
  vWaterArr_l3t5.push(...await getVelocityData(5, 3, 'water'))
  vWaterArr_l3t6.push(...await getVelocityData(6, 3, 'water'))
  vWaterArr_l3t7.push(...await getVelocityData(7, 3, 'water'))
  vWaterArr_l3t8.push(...await getVelocityData(8, 3, 'water'))

  featureEach(pressure_l1t1_jsonData, (currentFeature, featureIndex) => {
    currentFeature.properties.pressure = vWaterArr_l3t1[featureIndex]
  })

  //sample velocity array
  const vWaterArrList_l3: number[][] = [
    vWaterArr_l3t1,
    vWaterArr_l3t2,
    vWaterArr_l3t3,
    vWaterArr_l3t4,
  ...
  const mainRendering = () => {
    ...
    let dynamicRenderController = render.renderByTimeInterpolation(cesiumFieldMap, sampleData_l3, contourOptions, renderOptions)
  }
...
.cesiumContainer-threshold-water3 {
  position: relative;
  width: 100%;
  height: 99%;
