<template>
  <div class="cesiumContainer-threshold-oil3" id="cesiumContainer-threshold-oil3">
...
onMounted(async () => {
  /*--------initializing map------------*/
  let {viewer, scene, globe, clock} = CesiumTool.initMap("cesiumContainer-threshold-oil3")
  ...
  // layer3
  let vOilArr_l3t1 = reactive<number[]>([])
  let vOilArr_l3t2 = reactive<number[]>([])
  let vOilArr_l3t3 = reactive<number[]>([])
  let vOilArr_l3t4 = reactive<number[]>([])
  let vOilArr_l3t5 = reactive<number[]>([])
  let vOilArr_l3t6 = reactive<number[]>([])
  let vOilArr_l3t7 = reactive<number[]>([])
  let vOilArr_l3t8 = reactive<number[]>([])

  vOilArr_l3t1.push(...await getVelocityData(1, 3, 'oil'))
  vOilArr_l3t2.push(...await getVelocityData(2, 3, 'oil'))
  vOilArr_l3t3.push(...await getVelocityData(3, 3, 'oil'))
  vOilArr_l3t4.push(...await getVelocityData(4, 3, 'oil'))
  vOilArr_l3t5.push(...await getVelocityData(5, 3, 'oil'))
  vOilArr_l3t6.push(...await getVelocityData(6, 3, 'oil'))
  vOilArr_l3t7.push(...await getVelocityData(7, 3, 'oil'))
  vOilArr_l3t8.push(...await getVelocityData(8, 3, 'oil'))

  featureEach(pressure_l1t1_jsonData, (currentFeature, featureIndex) => {
    currentFeature.properties.pressure = vOilArr_l3t1[featureIndex]
  })

  //sample velocity array
  const vOilArrList_l3: number[][] = [
    vOilArr_l3t1,
    vOilArr_l3t2,
    vOilArr_l3t3,
    vOilArr_l3t4,
...
.cesiumContainer-threshold-oil3 {
  position: relative;
  width: 100%;
  height: 99%;
