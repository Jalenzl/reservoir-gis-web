<template>
  <div class="cesiumContainer-comp4" id="cesiumContainer-comp4">
...
onMounted(async () => {
  ...
  let {viewer, scene, globe, clock} = CesiumTool.initMap("cesiumContainer-comp4")
  ...
  const cesiumFieldMap = new CesiumFieldMap(
      viewer, // viewer
      pressure_l4t1_jsonData, // geojson
      clock, // clock
      "pressure", // propName
  )
    ...
    watch(
        ...
        (val) => {
          if (Number(val.renderType) === 1) {
            ...
            staticRenderController = render.renderByFixedTime(cesiumFieldMap, sampleData_l4, renderOptions)
          } else if (Number(val.renderType) === 2) {
            if (staticRenderController) {
              clearInterval(staticRenderController)
              viewer.entities.removeAll()
            }
            dynamicRenderController = render.renderByTimeInterpolation(cesiumFieldMap, sampleData_l4, renderOptions)
          }
        },
        {immediate: true}
    )
  ...
  const startSectionAnalysis = async () => {
    const sectionAnalysis: SectionAnalysis = new SectionAnalysis(viewer, clock, pressure_l4t1_jsonData, sampleData_l4)
  ...
  const startIsolineAnalysis = async () => {
    const centoidPoints = await GeoJsonTool.getCentroid(cesiumFieldMap, sampleData_l4, coordinatesArr, cesiumFieldMap.getTileNum())
  ...
  const dynamicChartAnalysis = new DynamicChartAnalysis(dynamicCharts.value as HTMLCanvasElement, cesiumFieldMap, clock, sampleData_l4)
  ...
  const viewModel = {
    showExtrudedHeight() {
      CesiumTool.showExtrudedHeight(cesiumFieldMap, sampleData_l4)
    },
    hideExtrudedHeight() {
